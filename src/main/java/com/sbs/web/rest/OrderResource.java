...
import com.sbs.repository.OrderRepository;
import com.sbs.service.OrderService;
...
/**
 * REST controller for managing {@link com.sbs.domain.Order}.
 */
@RestController
@RequestMapping("/api")
public class OrderResource {
    ...
    /**
     * {@code POST  /orders} : preview a new order form.
     *
     * @param order the order to create.
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new order, or with status {@code 400 (Bad Request)} if the order has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping(value = "/orders/preview", produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<byte[]> previewOrderForm(@RequestBody Order order) throws URISyntaxException, IOException {
        log.debug("REST request to generate and preview a Order Form with order details : {}", order);
        if (order.getBuyer() == null) {
            throw new BadRequestAlertException("Complete Order Information ", ENTITY_NAME, " buyerDoesn'tExist");
        }
        if (order.getSeller() == null) {
            throw new BadRequestAlertException("Complete Order Information ", ENTITY_NAME, " sellerDoesn'tExist");
        }
        if (order.getDate() == null) {
            throw new BadRequestAlertException("Complete Order Information ", ENTITY_NAME, " dateDoesn'tExist");
        }
        if (order.getItems() == null) {
            throw new BadRequestAlertException("Complete Order Information ", ENTITY_NAME, " itemsDoesn'tExist");
        }
        if (order.getTransport() == null) {
            throw new BadRequestAlertException("Complete Order Information ", ENTITY_NAME, " transportDoesn'tExist");
        }
        byte[] bytes = orderService.previewPdf(order);
        return ResponseEntity.created(new URI("/api/orders/preview")).body(bytes);
    }
